...
def get_data(size, total_sample_size):
    ...
    image = cv2.imread('Dataset_Recognition/s' + str(1) + '/' + str(1) + '.jpg')
    ...
    for i in range(3):
        for j in range(int(total_sample_size / 3)):
            ind1 = 0
            ind2 = 0
            ...
            # read the two images
            img1 = cv2.imread('Dataset_Recognition/s' + str(i + 1) + '/' + str(ind1 + 1) + '.jpg')
            img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
            img2 = cv2.imread('Dataset_Recognition/s' + str(i + 1) + '/' + str(ind2 + 1) + '.jpg')
    ...
    for i in range(int(total_sample_size / 10)):
        for j in range(10):
            ...
            img1 = cv2.imread('Dataset_Recognition/s' + str(ind1 + 1) + '/' + str(j + 1) + '.jpg')
            img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
...
def run():
    try:
        ...
        # print(X.shape)  #(200, 2, 1, 134, 134) --> 100 genuine, 100 imposite pairs
        # print(Y.shape)
        ...
        with open('mrezaPrepoznavanje', 'wb') as f:
            pickle.dump(model, f)
